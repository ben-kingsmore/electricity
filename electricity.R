...
agile_prices <- read_csv("C:/Users/benki/Desktop/battery_calcs/agile-half-hour-actual-rates-01-01-2024_25-09-2025.csv")
names(agile_prices) <- c("from", "to", "import", "export")

agile_prices <- agile_prices %>% 
  mutate(from = dmy_hm(from),
         to = dmy_hm(to),
...
df <- usage %>% 
  ...
  left_join(agile_prices %>% 
              select(from, import, export),
            by = "from") %>% 
  mutate(date = date(from),
  ...
  pivot_longer(c(consumption, import, export),
               names_to = "measure")
...
df %>% 
  filter(date %in% sample_dates,
         measure == "import") %>%
  mutate(price.centile = ecdf(value)(value),
         .by = date) %>% 
...
df %>% 
  ...
  mutate(price.centile = ecdf(import)(import),
         .by = date) %>% 
  mutate(price.bucket = cut(price.centile, c(0, 0.25, 0.5, 0.75, 1),
                            include.lowest = TRUE)) %>% 
  summarise(total.cons = sum(consumption),
  ...
  summarise(total.cons = sum(total.cons),
            .by = c(date, above.5))
  summarise(share.above.5 = sum(above.5) / n(),
            .by = month)
  ggplot(aes(x = month, y = mean.kwh, colour = price.bucket, group = price.bucket)) +
  geom_line() +
  geom_point()
